...
def run(ckpt,model_name,dynamic,soft, eval, model_type, noisy):
    ...
    if not eval:
        if model_type == 'DDPG':
            train_ddpg(ckpt, model_name, dynamic, soft)
        else:
            train_dqn(ckpt, model_name, dynamic, noisy)

    else:
        if ckpt:
            brain = torch.load(ckpt,map_location=torch.device('cpu'), weights_only=False)
            brain.epsilon = 0
            brain.eps_end = 0
            ...
            for inside_temp in np.arange(19, 23, 1/10):
                for ambient_temp in np.arange(-5, 5, 1/10):
                    for price in range(0, 60):
                        state = [inside_temp, ambient_temp, 0, price, SOC_INIT]
                        state = torch.tensor(state, dtype=torch.float).to(device)
                        state = brain.normalizer.normalize(state).unsqueeze(0)
                        action = int(brain.select_action(state).item())
